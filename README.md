...
2. **与已选课程冲突是硬门槛**：`IS_CONFLICT===1` 的班级直接排除（教务会拒/撞课）。
   所以当“最优”刚好冲突、“次优”不冲突时，会自动选那个不冲突的，而不是硬抢冲突班；
   全校区都冲突时干脆不选并告警。
3. **任课教师**：符合教师的班级整档优先。
4. **上课时间地点**：没有符合教师的班级时，符合时间的优先；都不符合的排最后。
5. 同一档内：**有空位优先** → 教师关键词位置（越靠前越优：`张三,李四` ＞ `李四,张三`）→ 时间位置 → 班级代码。
6. 高优先档全满时**等它出空位**，不会改选低优先档。

`allowFallback`（默认 `true`）：优先档不存在/全满时按 3 → 4 降级；写 `false` 则只选教师+时间都符合的班。
`avoidConflict`（默认 `true`）：排除冲突班；写 `false` 才允许选冲突班（一般没必要）。
`--dry` 会打印 `候选N 校区内N 不冲突N 符合条件N 有空位N 最优=... 首选=...`，可直接核对。
